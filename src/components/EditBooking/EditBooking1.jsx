...
function EditBooking1({}) {
    ...
    useEffect(() => {
      const fetchBookingDetails = async () => {
        try {
          ...
          const response = await fetch(`${BASE_URL}/booking1/${id}`, {
            method: 'GET',
            headers: {
          ...
          if (response.ok) {
            ...
            const hotelId = bookingdata.data.hotelId; // Make sure to adjust this based on your data structure
            const hotelResponse = await fetch(`${BASE_URL}/hotels/${hotelId}`, {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token}`,
            ...
            //   setTour(hotelData.data);
              const hotelPrice = hotelData.data.price; // Adjust based on the actual structure
              setHotel({ ...hotelData.data, price: hotelPrice });

            } else {
              console.error('Failed to fetch hotel details:', hotelResponse.statusText);
          } 
        }
          else {
    ...
    const totalAmount = Number(hotel.price) * Number(booking.guestSize) + Number(serviceFee);
    ...
    const handleClick = async (e) => {
      ...
      try {
        ...
        const res = await fetch(`${BASE_URL}/booking1/${id}`, {
  ...
  return (
    <div className='bookingg'>
       <div className="booking__top d-flex align-items-center justify-content-between">
          <h3>Rs{hotel.price} <span>/per person</span></h3>
       ...
       <div className="booking__bottom">
          <ListGroup>
             <ListGroupItem className='border-0 px-0'>
                <h5 className='d-flex align-items-center gap-1'>Rs{hotel.price} <i class='ri-close-line'></i> 1 person</h5>
