...
function EditBooking2({}) {
    ...
    useEffect(() => {
      const fetchBookingDetails = async () => {
        try {
          ...
          const response = await fetch(`${BASE_URL}/booking2/${id}`, {
            method: 'GET',
            headers: {
          ...
          if (response.ok) {
            ...
            const flightId = bookingdata.data.flightId; // Make sure to adjust this based on your data structure
            const flightResponse = await fetch(`${BASE_URL}/flights/${flightId}`, {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token}`,
            ...
            //   setTour(flightData.data);
              const flightPrice = flightData.data.price; // Adjust based on the actual structure
              setFlight({ ...flightData.data, price: flightPrice });
    ...
    const totalAmount = Number(flight.price) * Number(booking.guestSize) + Number(serviceFee);
    ...
    const handleClick = async (e) => {
      ...
      try {
        ...
        const res = await fetch(`${BASE_URL}/booking2/${id}`, {
  ...
  return (
    <div className='bookingg'>
       <div className="booking__top d-flex align-items-center justify-content-between">
          <h3>Rs{flight.price} <span>/per person</span></h3>
       ...
       <div className="booking__bottom">
          <ListGroup>
             <ListGroupItem className='border-0 px-0'>
                <h5 className='d-flex align-items-center gap-1'>Rs{flight.price} <i class='ri-close-line'></i> 1 person</h5>
